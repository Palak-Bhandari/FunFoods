...
{ DS_PRODUCT.PRODUCTMST_SELECTDataTable PDT = new DS_PRODUCT.PRODUCTMST_SELECTDataTable();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
         if (Page.IsPostBack == false)
        {
            ...
            if (idd == "1")
            {
                PDT = PAdapter.SELECT_BY_CNAME("GRILL");
                DataList1.DataSource = PDT;
                DataList1.DataBind();
            }
            else if (idd == "2")
            {
                PDT = PAdapter.SELECT_BY_CNAME("TEA");
                DataList1.DataSource = PDT;
                DataList1.DataBind();
            ...
            {
                PDT = PAdapter.SELECT_BY_CNAME("PASTA");
                DataList1.DataSource = PDT;
                DataList1.DataBind();
            }
            else if (idd == "4")
            {
                PDT = PAdapter.SELECT_BY_CNAME("VEG PIZZA");
                DataList1.DataSource = PDT;
                DataList1.DataBind();
            }
            else if (idd == "5")
            {
                PDT = PAdapter.SELECT_BY_CNAME("VEG BURGER");
                DataList1.DataSource = PDT;
                DataList1.DataBind();
            }
            else if (idd == "6")
            {
                PDT = PAdapter.SELECT_BY_CNAME("WRAPS");
                DataList1.DataSource = PDT;
                DataList1.DataBind();
            }
            else if (idd == "7")
            ...
            {
                PDT = PAdapter.SELECT_BY_CNAME("FRIES");
                DataList1.DataSource = PDT;
                DataList1.DataBind();
            }
            else if (idd == "9")
            {
                PDT = PAdapter.SELECT_BY_CNAME("COFFEE");
                DataList1.DataSource = PDT;
                DataList1.DataBind();
            }
